...
class Hyperparameters:
    ...
    num_layers = int(os.environ.get("NUM_LAYERS", 51))
    ...
    # Weight sharing: num_unique_layers unique blocks, cycled num_layers times
    # e.g. 4 unique blocks cycled 3x = 12 virtual layers with 4 blocks' params
    num_unique_layers = int(os.environ.get("NUM_UNIQUE_LAYERS", 17))  # 0 = no sharing (all unique)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Weight sharing: create num_unique_layers blocks, cycle through them num_layers times
        n_unique = num_unique_layers if num_unique_layers > 0 else num_layers
        self.num_unique = n_unique
        ...
        # Per-virtual-layer mixing weights (unique per layer even when sharing blocks)
        # These are cheap (2*dim per layer) and help differentiate shared passes
        if num_unique_layers > 0 and num_unique_layers < num_layers:
            self.layer_mix = nn.ParameterList([
    ...
    def _get_block(self, virtual_idx: int) -> nn.Module:
        return self.blocks[virtual_idx % self.num_unique]
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            if self.layer_mix is not None:
                mix = self.layer_mix[i].to(dtype=x.dtype)
                x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
            x, v_new = self._get_block(i)(x, x0, v0=v0)
            if v0 is None:
                v0 = v_new
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            if self.layer_mix is not None:
                mix = self.layer_mix[vi].to(dtype=x.dtype)
                x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
            x, _ = self._get_block(vi)(x, x0, v0=v0)
...
def main() -> None:
    ...
    if base_model.layer_mix is not None:
        for lm in base_model.layer_mix:
            scalar_params.append(lm)
