...
class Hyperparameters:
    ...
    num_layers = int(os.environ.get("NUM_LAYERS", 17))
    ...
    # Partial RoPE: apply rotary to only this many dims per head (0 = all)
    rope_dims = int(os.environ.get("ROPE_DIMS", 16))

    # LN Scale: scale RMSNorm outputs by 1/sqrt(layer_idx+1)
    ln_scale = bool(int(os.environ.get("LN_SCALE", "1")))

    # Late QAT: enable STE fake-quantization in the last fraction of training
    ...
    ttt_phase2_epochs = int(os.environ.get("TTT_PHASE2_EPOCHS", 10))
    ttt_phase2_lr = float(os.environ.get("TTT_PHASE2_LR", 0.005))
    ttt_phase2_unfreeze_last = int(os.environ.get("TTT_PHASE2_UNFREEZE_LAST", 3))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,bigram.scale,v_res_alpha,attn_gate",
    ).split(",")
    if pattern
)
...
def quantize_int5_per_row(t: Tensor) -> tuple[Tensor, Tensor]:
    """Int5 [-16, 15]: 3 zero high bits per byte → great zstd compression."""
    t32 = t.float()
    if t32.ndim == 2:
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], int5_cats: set[str] = set()):
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float, qk_gain_init: float, rope_dims: int = 0, use_xsa: bool = False, value_residual: bool = False):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.v_res_alpha = nn.Parameter(torch.tensor(0.0, dtype=torch.float32)) if value_residual else None
    ...
    def forward(self, x: Tensor, v0: Tensor | None = None) -> tuple[Tensor, Tensor]:
        ...
        v_cache = v
        # Value Residual (ResFormer): mix in V from first layer
        if self.v_res_alpha is not None and v0 is not None:
            alpha = torch.sigmoid(self.v_res_alpha.to(v.dtype))
        ...
        if self.use_xsa:
            v_self = v.repeat_interleave(self.num_heads // self.num_kv_heads, dim=1) if self.num_kv_heads != self.num_heads else v
            dot = (y * v_self).sum(dim=-1, keepdim=True)
            norm_sq = (v_self * v_self).sum(dim=-1, keepdim=True).clamp_min(1e-8)
            y = y - (dot / norm_sq) * v_self
        # Gated Attention: per-head sigmoid gate
        gate = torch.sigmoid(self.attn_gate.to(dtype=y.dtype))[None, :, None, None]
        y = y * gate
        y = y.transpose(1, 2).contiguous().reshape(bsz, seqlen, dim)
        return self.proj(y), v_cache
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: float,
                 ...
                 use_xsa: bool = False, ln_scale: float = 1.0, value_residual: bool = False):
        super().__init__()
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init,
                                         rope_dims=rope_dims, use_xsa=use_xsa, value_residual=value_residual)
    ...
    def forward(self, x: Tensor, x0: Tensor, v0: Tensor | None = None) -> tuple[Tensor, Tensor]:
        ...
        attn_out, v_new = self.attn(normed, v0=v0)
        x = x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * attn_out
        normed_mlp = self.mlp_norm(x)
        if self.ln_scale != 1.0:
            normed_mlp = normed_mlp * self.ln_scale
        x = x + self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(normed_mlp)
        return x, v_new
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            x, v_new = self.blocks[i](x, x0, v0=v0)
            if v0 is None:
                v0 = v_new
            skips.append(x)
...
def main() -> None:
    ...
    optimizer_muon = Muon(
        ...
        weight_decay=float(os.environ.get("MUON_WD", 0.04)),
    )
    ...
    while True:
        ...
        # Late QAT: activate STE fake-quantization when LR schedule drops
    ...
    # ========================================
    # TWO-PHASE TEST-TIME TRAINING (TTT)
    # ========================================
    # Phase 1: Norm-Only Recalibration — fix activation distributions damaged by int6 quant
    # Phase 2: Selective-Freeze Block Adaptation — fine-tune last N blocks on val tokens already evaluated
    if args.ttt_enabled:
        log0("ttt:starting Two-Phase Test-Time Training")
        # Use a subset of validation tokens for TTT (tokens we'll evaluate on)
        ttt_tokens = val_tokens[:min(val_tokens.numel(), args.train_seq_len * 64 + 1)].to(device=device, dtype=torch.int64)
        ttt_x = ttt_tokens[:-1].reshape(-1, args.train_seq_len)
        ttt_y = ttt_tokens[1:].reshape(-1, args.train_seq_len)

        # Phase 1: Norm-Only Recalibration (Adam, small params only)
        log0(f"ttt:phase1 norm_recalibration epochs={args.ttt_phase1_epochs} lr={args.ttt_phase1_lr}")
        norm_params = []
        for name, p in base_model.named_parameters():
            p.requires_grad_(False)
        for name, p in base_model.named_parameters():
            if 'norm' in name or 'scale' in name or 'attn_gate' in name or 'v_res_alpha' in name:
                p.requires_grad_(True)
                norm_params.append(p)
        if norm_params:
            ttt_opt1 = torch.optim.Adam(norm_params, lr=args.ttt_phase1_lr)
            base_model.train()
            for epoch in range(args.ttt_phase1_epochs):
                ttt_opt1.zero_grad(set_to_none=True)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    loss = base_model(ttt_x, ttt_y)
                loss.backward()
                ttt_opt1.step()
                if (epoch + 1) % 10 == 0 or epoch == 0:
                    log0(f"  ttt:phase1 epoch:{epoch+1}/{args.ttt_phase1_epochs} loss:{loss.item():.4f}")
            del ttt_opt1

        # Phase 2: Selective-Freeze Block Adaptation (SGD, unfreeze last N blocks)
        log0(f"ttt:phase2 block_adaptation epochs={args.ttt_phase2_epochs} lr={args.ttt_phase2_lr} unfreeze_last={args.ttt_phase2_unfreeze_last}")
        for name, p in base_model.named_parameters():
            p.requires_grad_(False)
        num_blocks = len(base_model.blocks)
        phase2_params = []
        for i in range(max(0, num_blocks - args.ttt_phase2_unfreeze_last), num_blocks):
            for p in base_model.blocks[i].parameters():
                p.requires_grad_(True)
                phase2_params.append(p)
        if phase2_params:
            ttt_opt2 = torch.optim.SGD(phase2_params, lr=args.ttt_phase2_lr)
            for epoch in range(args.ttt_phase2_epochs):
                ...
                if (epoch + 1) % 5 == 0 or epoch == 0:
                    log0(f"  ttt:phase2 epoch:{epoch+1}/{args.ttt_phase2_epochs} loss:{loss.item():.4f}")
            del ttt_opt2
    ...
    # Sliding window eval on int6-roundtripped (+ TTT) weights
    torch.cuda.synchronize()
