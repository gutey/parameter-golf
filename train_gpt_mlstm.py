...
# Try to import optimized TFLA Triton kernels for mLSTM
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,skip_weight,skip_weights,head_norm_weight,igate,fgate,smear,bigram.scale",
    ).split(",")
    if pattern
)
...
def _classify_param(name: str) -> str:
    ...
    if ".mlp." in name:
        return "mlp"
    if ".mlstm." in name or ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
...
def mlstm_parallel(q: Tensor, k: Tensor, v: Tensor, i: Tensor, f: Tensor, eps: float = 1e-6) -> Tensor:
    """Native PyTorch mLSTM parallel forward (quadratic in seq_len).
    q, k: (B, NH, S, DHQK)
    v: (B, NH, S, DHV)
    i, f: (B, NH, S) - input/forget gate pre-activations
    Returns: (B, NH, S, DHV)
    """
    B, NH, S, DHQK = q.shape
    # Log-domain gates for numerical stability
    log_f = F.logsigmoid(f)  # (B, NH, S)
    log_f_cumsum = log_f.cumsum(-1)  # (B, NH, S)
    mat_log_f = log_f_cumsum[:, :, :, None] - log_f_cumsum[:, :, None, :]  # (B, NH, S, S)
    # Causal mask
    ltr = torch.tril(torch.ones(S, S, dtype=torch.bool, device=q.device))
    mat_log_f = torch.where(ltr, mat_log_f, torch.tensor(-float("inf"), device=q.device, dtype=mat_log_f.dtype))
    mat_log_d = mat_log_f + i[:, :, None, :]  # (B, NH, S, S)
    # Stabilize with max
    vec_m, _ = torch.max(mat_log_d, dim=-1, keepdim=True)  # (B, NH, S, 1)
    mat_d = torch.exp(mat_log_d - vec_m)  # (B, NH, S, S)
    # QK similarity
    mat_s = (q @ k.transpose(-2, -1)) * (DHQK ** -0.5)  # (B, NH, S, S)
    mat_c_tilde = mat_s * mat_d  # (B, NH, S, S)
    # Normalizer
    vec_n = torch.maximum(
        mat_c_tilde.sum(dim=-1, keepdim=True).abs(),
        torch.exp(-vec_m)
    )  # (B, NH, S, 1)
    mat_c = mat_c_tilde / (vec_n + eps)  # (B, NH, S, S)
    return mat_c @ v  # (B, NH, S, DHV)


def mlstm_forward(q: Tensor, k: Tensor, v: Tensor, i: Tensor, f: Tensor,
                  chunk_size: int = 256, eps: float = 1e-6) -> Tensor:
    """Dispatch to TFLA Triton kernels if available, else native parallel."""
    if _HAS_MLSTM_KERNELS:
        # TFLA xl_chunk kernel: much faster, sub-quadratic via chunking
        ...
        return h
    return mlstm_parallel(q, k, v, i, f, eps=eps)


class mLSTMLayer(nn.Module):
    """mLSTM layer: replaces CausalSelfAttention.
    Uses exponential input gating + sigmoid forget gating with matrix memory.
    """
    def __init__(self, dim: int, num_heads: int, gate_soft_cap: float = 15.0, chunk_size: int = 256):
        super().__init__()
        self.num_heads = num_heads
        self.head_dim = dim // num_heads
        # qk uses half the head dim for efficiency (like xLSTM 7B: qk_dim_factor=0.5)
        self.qk_dim = dim // 2
        ...
        self.igate = nn.Linear(dim, num_heads, bias=True)
        self.fgate = nn.Linear(dim, num_heads, bias=True)
        # Output gate: per v_dim
        self.ogate = CastedLinear(dim, self.v_dim, bias=False)
        # Output projection
        self.proj = CastedLinear(self.v_dim, dim, bias=False)
        self.proj._zero_init = True
        # Per-head layer norm on output
        self.head_norm_weight = nn.Parameter(torch.ones(num_heads, self.head_dim))

    def _soft_cap(self, x: Tensor) -> Tensor:
        return self.gate_soft_cap * torch.tanh(x / self.gate_soft_cap)

    def forward(self, x: Tensor) -> Tensor:
        bsz, seqlen, dim = x.shape
        q = self.c_q(x).reshape(bsz, seqlen, self.num_heads, self.qk_head_dim).transpose(1, 2)
        k = self.c_k(x).reshape(bsz, seqlen, self.num_heads, self.qk_head_dim).transpose(1, 2)
        v = self.c_v(x).reshape(bsz, seqlen, self.num_heads, self.head_dim).transpose(1, 2)
        # Gates with soft capping
        i_preact = self._soft_cap(self.igate(x)).transpose(1, 2)  # (B, NH, S)
        f_preact = self._soft_cap(self.fgate(x)).transpose(1, 2)  # (B, NH, S)
        # mLSTM forward (TFLA kernels if available, else native parallel)
        h = mlstm_forward(q, k, v, i_preact, f_preact, chunk_size=self.chunk_size)
        # Per-head layer norm
        h = F.layer_norm(h, (self.head_dim,))
        h = h * self.head_norm_weight.to(dtype=h.dtype)[None, :, None, :]
        h = h.transpose(1, 2).contiguous().reshape(bsz, seqlen, self.v_dim)
        # Output gating
        o = torch.sigmoid(self.ogate(x))
        h = o * h
        return self.proj(h)
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlstm = mLSTMLayer(dim, num_heads, gate_soft_cap=gate_soft_cap, chunk_size=mlstm_chunk_size)
    ...
    def forward(self, x: Tensor, x0: Tensor, resid_w: float = 1.0, x0_w: float = 0.0) -> Tensor:
        x = resid_w * x + x0_w * x0
        x = self.smear(x)
        mlstm_out = self.mlstm(self.attn_norm(x))
        x = x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * mlstm_out
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        for block in self.blocks:
            if hasattr(block.mlstm, 'proj'):
                block.mlstm.proj.weight.data *= proj_scale
